...
for VERSION in "${VERSION_ARRAY[@]}"; do
  {
    ...
    # Iterate over each input file in parallel
    for INPUT_FILE in "${INPUT_FILES[@]}"; do
      ...
      } &  # Run the input file processing in the background
    done  
    wait  # Wait for all background processes to finish
  } &  # Run the version processing in the background
done

wait  # Wait for all version processing to finish
